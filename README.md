...
- **`BLOGGER_MAX_NEW_POSTS_PER_DAY` (기본값 50)**: Blogger API의 "신규 글 발행"에 대해
  ...
  흔합니다. 그래서 이 값은 "미리 포기하고 시도조차 안 할" 상한선으로만 쓰이고, 실제로 403/429를
  받으면 그 즉시 그날의 실질 한도로 인식해 남은 주제는 신규 발행 대신 가장 오래된 글 업데이트로
  전환합니다. 다음 실행(4시간 뒤)에는 다시 새로 판단하므로, 블로그가 신뢰도를 쌓아 한도가
  완화되면 코드 수정 없이 자동으로 반영됩니다.
