...
def find_empty_shell(service, blog_id: str) -> dict | None:
    """Backward-compatible helper: empty DRAFT/LIVE titles only."""
    for status in ("DRAFT", "LIVE"):
        for post in _list_posts(service, blog_id, status, limit=50):
...
def find_reusable_shell(service, blog_id: str) -> dict | None:
    """Prefer any DRAFT (factory/low-value first), else empty LIVE shell only."""
    drafts = _list_posts(service, blog_id, "DRAFT", limit=50)
    live = _list_posts(service, blog_id, "LIVE", limit=50)
    live_titles = {(p.get("title") or "").strip() for p in live}

    def draft_score(post: dict) -> tuple:
        title = (post.get("title") or "").strip()
        content = post.get("content") or ""
        text = re.sub(r"<[^>]+>", "", content).strip()
        factory = 1 if ("확인 방법" in title or "체크리스트" in title) else 0
        low_value = 1 if ("뭐길래" in title or len(text) < 2200 or len(post.get("labels") or []) < 15) else 0
        dup_live = 1 if title in live_titles else 0
        # Higher first: duplicate-of-live, factory, low-value, then shorter body.
        return (-dup_live, -factory, -low_value, len(text), title)

    usable_drafts = [
        p
        for p in drafts
        if (p.get("title") or "").strip() not in PROTECTED_TITLES
    ]
    if usable_drafts:
        usable_drafts.sort(key=draft_score)
        return usable_drafts[0]

    # Empty LIVE shells only — never overwrite a healthy live post.
    for post in live:
        title = (post.get("title") or "").strip()
        content = post.get("content") or ""
        text = re.sub(r"<[^>]+>", "", content).strip()
            ...
            try:
                return (
                    ...
                    .patch(
                        blogId=blog_id,
                        postId=post_id,
                        body={"published": now_iso, "labels": labels},
                    )
                    .execute()
                )
            except Exception:
